...
def visualize_tsne(images, labels, class_names, model):
    ...
    plt.title('t-SNE Visualization of TNC Algorithm Features')
...
def main(train_loader, valid_loader, valid_balanced_dataloader, seed):
    ...
    for epoch in tqdm(range(1, num_epochs+1)):
        ...
        for batch_idx, (time_series, _) in enumerate(train_loader):
            ...
            x_t = time_series[:,t-window_size//2:t+window_size//2]
            
            x_p, delta = find_neighours(time_series, t, window_size)
            ...
            train_loss = (p_loss + w*n_loss_u + (1-w)*n_loss)/2
        ...
        if epoch % config.VALID_INTERVAL == 0:
            ...
            if epoch % config.SAVE_INTERVAL == 0:
                torch.save(attn_model.state_dict(), f'models/{ds_name + str(seed)}_tnc_model_epoch_{epoch}.pth')

    if config.WANDB:
        wandb.finish()
        
    print("TNC Training and validation complete.")
    print("|")
