...
def visualize_tsne(images, labels, class_names, model):
    ...
    plt.title('t-SNE Visualization of MSE Reconstruction Features')
    ...
    for epoch in tqdm(range(1, num_epochs+1)):
        ...
        if epoch % config.VALID_INTERVAL == 0:
            ...
            if epoch % config.SAVE_INTERVAL == 0:
                torch.save(attn_model.state_dict(), f'models/{ds_name + str(seed)}_recons_model_epoch_{epoch}.pth')
