...
def visualize_tsne(images, labels, class_names, model):
    ...
    plt.title('t-SNE Visualization of Triplet Features')
...
def main(train_loader, valid_loader, valid_balanced_dataloader, seed):
    ...
    triplet_loss = TripletLoss(compared_length=100, nb_random_samples = 10, negative_penalty=1, output_size=args['out_features'])
    ...
    for epoch in tqdm(range(1, num_epochs+1)):
        ...
        for batch_idx, (time_series, _) in enumerate(train_loader):
            ...
            train_loss = triplet_loss(time_series, attn_model, train, save_memory=False)
            train = time_series
        ...
        if epoch % config.VALID_INTERVAL == 0:
            ...
            if epoch % config.SAVE_INTERVAL == 0:
                torch.save(attn_model.state_dict(), f'models/{ds_name + str(seed)}_triplet_model_epoch_{epoch}.pth')


    if config.WANDB:
        wandb.finish()
        
    print("Triplet Training and validation complete.")
    print("|")
