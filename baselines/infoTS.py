...
def visualize_tsne(images, labels, class_names, model):
    ...
    plt.title(f't-SNE Visualization of InfoTS Algorithm Features')
    ...
    for epoch in tqdm(range(1, num_epochs+1)):
        ...
        for batch_idx, (x, _) in enumerate(train_loader):
            ...
            if num_epochs==-1:
                ...
                t = float(t0 * np.power(t1 / t0, (num_epochs+1) / num_epochs))

            a1,a2 = aug((x,t))

            out1 = attn_model(a1.to(device))
            out2 = attn_model(a2.to(device))

            # Calculate the loss
            train_loss = global_infoNCE(
        ...
        if epoch % config.VALID_INTERVAL == 0:
            ...
            if epoch % config.SAVE_INTERVAL == 0:
                torch.save(attn_model.state_dict(), f'models/{ds_name + str(seed)}_infoTS_model_epoch_{epoch}.pth')
