...
from src.losses.contrastive import LS_HATCL_LOSS, MARGIN_LOSS, HATCL_LOSS, LS_MARGIN_LOSS
...
def check_next_two_elements_tensor(tensor, thresh):
    """
...
def main(ssl_loader, valid_loader):
    ...
    ls_margin_loss = LS_MARGIN_LOSS(temperature=0.1)
    thresh = 0.4
    # mse_lambda = 0.001
    mask_fraction = 0.2  # For example, 10% of the elements will be masked
    ...
    for epoch in range(num_epochs):
        ...
        for time_series in tqdm(ssl_loader):
            ...
            sim_vector = check_next_two_elements_tensor(time_series, thresh)
            ...
            masked_features = masked_features.reshape(-1, features.size(-1))
            
            # Compute training loss
            contrastive_loss = ls_margin_loss(features, sim_vector)
            mse_loss = F.mse_loss(features, masked_features)
            
            train_loss = contrastive_loss + mse_loss
        ...
        if epoch % 2 == 0:
            ...
            for time_series in tqdm(valid_loader):
                ...
                valid_loss = contrastive_loss + mse_loss
